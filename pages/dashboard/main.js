       ...
       window.history.replaceState({}, document.title, window.location.pathname);
        ...
        supaClient.functions.invoke('list-rsvp', { method: 'GET' }).then(({ data, error }) => {
          if (data && data.length > 0) {
            $app.components.page_data.store.guests = data.slice(0, 3);
          }
        });
...
function initTimerCountdown() {
	const timerCountdown = () => {
  	...
  	var target_date = new Date($app.components.page_data.store.eventDate).getTime();
...
function copyToClipboard() {
  ...
  navigator.clipboard.writeText(link).then(() => {
   ...
   tooltips.forEach((tooltip, index) => {
     ...
     setTimeout(() => { tooltip.style.display = "none"; }, 3000); // hide after 3 sec
   })   
     ...
     button.addEventListener("click", async () => {
      ...
      try {
       ...
       const { data, error } = await supaClient.functions.invoke('update-template-data', {
    	   body: JSON.stringify({ data: {slug, 'main-date': $app.components.page_data.store.rawEventDate} })
	     })
       if(error){
          throw new Error("Could not update slug");
        }
       ...
       window.location.href = "/dashboard";
        ...
        errorTag.textContent = "שגיאה במהלך השמירה, וודאו כי יש רק אותיות באנגלית ומספרים. ייתכן והשם תפוס, נסו אולי משהו אחר";
      }
    });
