<script src="https://cdn.jsdelivr.net/gh/Dkranc/weddji-pages-js/pages/edit/main.js"></script>
...
<script>
  ...
  supaClient.auth.getUser().then(({ data }) => { 
    ...
    user.user_metadata && Object.assign($app.components.page_data.store.user.metadata, user.user_metadata);
  });
  ...
  //pre- populate with template cms item saved data
   supaClient.functions.invoke('get-invitation-details', { method: 'GET' }).then(({ data, error }) => {
    if (error) {
      console.log(error);
      return;
    }
    ...
    if (!values.length) return;
     ...
     for (const [key, value] of values) {
      ...
      const elements = document.getElementsByName(key);
         if(elements && key !== 'slug' && key !== 'custom-image-24' && key !== 'wedding-date-en'){
  ...
  supaClient.functions.invoke('get-template-raw-data', { method: 'GET' }).then(({ data, error }) => {
    if (error) {
      console.log(error);
      return;
    }
...
function updateWhatsAppImage(imageUrl) {
  ...
  async function saveAndRefreash(e) {
    ...
    $app.components.page_data.store.weddingForm.saveAndRefreshBtnTxt = 'ההזמנה מתעדכנת';
    const { data, error } = await supaClient.functions.invoke('update-template-data', {
    	body: JSON.stringify({ data: templateData })
	  })
    $app.components.page_data.store.weddingForm.saveAndRefreshBtnTxt = 'שמור ועדכן הזמנה';
    if (!error) {
      updateIframeTemplate();
  ...
  document.querySelector('#Invite-details-form').addEventListener('submit', saveAndRefreash);
  ...
  async function completeEdit(e) {
    ...
    if (user.is_anonymous || !user.metadata.isPaying) {
      const finishedQuestions = $app.components.page_data.store.weddingForm.finishedQuestions;
  ...
  document.querySelector('#wf-form-feedback').addEventListener('submit', completeQuestions);
...
function scaleIframes() {
  try {
    ...
    const mobileContainers = document.querySelectorAll('.iframe-container.mobile-container');
...
document.querySelectorAll('.desktop-container, .iframe-container.mobile-container').forEach(container => {
...
<script>
  ...
  document.getElementById('whatsapp-title').addEventListener('input', function() {
    updateText('whatsapp-title', 'whatsapp-title-text');
  });
  document.getElementById('whatsapp-description').addEventListener('input', function() {
...
<script>
  ...
  document.querySelectorAll('.color-swatch:not([style])').forEach(el => el.style.display = 'none');
  ...
  const defaultChecked = document.querySelector('input[name="color-palette"]:checked');
  if (defaultChecked) {
...
document.querySelectorAll('uc-upload-ctx-provider').forEach(ctx => {
	const successIcon = ctx.closest('.form_field-wrapper').querySelector('.image-upload-validation');
...
supaClient.auth.getSession().then(({ data, error }) => {
  if (error || !data || !data.session) window.location.href = '/log-in';
     ...
     supaClient.functions.invoke('get-template-metadata', { method: 'GET' }).then(({ data, error }) => {
     if (error) {
     console.log(error);
     return;
  }
  ...
  async function getPaymentLink(price) {
    ...
    const { data, error } = await supaClient.functions.invoke('get-payment-link', {
